...
**FR-PR-03 (MUST)** *(amended 2026-05-19 per triage T-3)* — Each preset MUST carry a `label`, `description`, and `defaults` block. `defaults` MAY contain any subset of: `provider`, `model`, `temperature`, `top_p`, `max_sentences_per_chunk`, `normalize_db`, `response_format`, `postprocess` (object with `rms_normalize: bool`, `silence_trim: bool`, `denoise: bool`), `language`, `number_lang`, `voice` (default voice id when caller omits one). Unspecified fields fall through to existing system defaults. Fields intentionally NOT settable in a preset: `input` (the synthesis text — per-request only) and `stream` (the streaming toggle — per-request behavior, not a preset attribute).
...
| A-PR-1 (*cycle 2*, **revised per PO Phase 3 C-1**) | Cycle-2's `balanced` preset is operator-tunable. Cross-cycle byte-identity (cycle-1 default behaviour → cycle-2 default behaviour) is **NOT promised** (per NFR-OP-07 — no migration deliverable). Operators who require exact byte-compat with cycle-1 defaults are responsible for setting `balanced.defaults` to match. Same-cycle horizontal byte-identity (rich-with-balanced ↔ OpenAI server-default) **IS preserved** as a load-bearing invariant (NFR-PT-05). | If wrong (operators expect implicit cross-cycle byte-compat): UAT-PR-06 still verifies same-cycle horizontal byte-identity. Cross-cycle observability is operator-driven via S-021 perf comparison. |
