...
def test_health_returns_ok(client: TestClient) -> None:
    ...
    # S-006: provider self-report is part of ``/health``.
...
def test_health_without_provider_selection_still_returns_ok(
    ...
    """When lifespan is bypassed and no provider_selection slot is set, the
    liveness probe must still return 200 (it's the *liveness* probe — it
    cannot 5xx merely because optional self-report data is absent)."""
    from llm_tts_api.main import TEST_BYPASS_ENV, create_app

    monkeypatch.setenv(TEST_BYPASS_ENV, "1")
    app = create_app()
    # Intentionally do NOT populate app.state.provider_selection.
